...
| # | 코드 | 수학식 | 값 |
|:-:|--------------|------------------|----------------------|
|①|`t2 = pow(2, pow(cur, d, n), r)`|$2^{t_k}\pmod r$|$\chi(2)=-1$ → $(-1)^{t_k}$|
|②|`t3 = pow(3, pow(n, cur, d), r)`|$3^{\,n^{t_k}\! \bmod d}\pmod r$|$\chi(3)=+1$|
|③|`t4 = pow(4, pow(d, n, cur), r)`|$4^{\,d^{\,n}\! \bmod t_k}\pmod r$|$\chi(4)=+1$|
